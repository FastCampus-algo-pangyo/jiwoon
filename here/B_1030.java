...
public class B_1030 {
    ...
    static char[][] borad;
    private static void solve(int now, int byeon_i, int byeon_j, int size) {
        if (now < 1) return;
        if (byeon_i+ size < r1 || byeon_i > r2 || byeon_j + size < c1 || byeon_j > c2) return;

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                int nxt_i = byeon_i + i*size/n; //다음 변의 시작 i
                int nxt_j = byeon_j + j*size/n; //다음 변의 시작 j
                int nxt_size = size/n;

                if ((n%2==0 && is_center_even_ver(i,j)) || (n%2!=0 && is_center_odd_ver(i,j))) {
                    fill_black(nxt_i, nxt_j, nxt_size); //검정색을 칠해야하는 범위라면,
                } else if (nxt_i <= r2 && nxt_j <= c2){ //불필요한 재귀를 막기 위한.... ㅜ
    ...
    private static void fill_black(int size_i, int size_j, int size) {
        if (size_i < r1 || size_i+size > r2 || size_j < c1 || size_j + size > c2) return;
